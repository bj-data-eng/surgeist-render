...
#[cfg_attr(
    ...
    expect(
        unused_imports,
        reason = "the staged pass front door preserves its crate-visible closure contract"
    )
)]
pub(crate) use close::{
...
#[cfg(test)]
use close::{
    ClosedExecutableGraph, ClosedExecutableGraphFacts, ExecutableColorFilterFacts,
    ExecutableLayerCompositionFacts, PrePreparationGraphClassification,
    c08_resource_has_fixed_facts, executable_vello_capture_facts,
};
#[cfg(test)]
use close::{ExecutableFilterStepFacts, preparation_error};
use encode::C08CustomSpineEncodingState;
#[cfg_attr(
    not(test),
    expect(
        unused_imports,
        reason = "the staged pass front door preserves its crate-visible encoding contract"
    )
)]
...
#[cfg(test)]
use encode::{C08ScheduledEncodingKind, backdrop_filter_passes, vello_capture_raster_parameters};
#[expect(
    unused_imports,
    reason = "the staged pass front door preserves its crate-visible capture contract"
)]
pub(crate) use encode::{C08VelloCaptureCompletionReceipt, C08VelloCaptureEncodingHandoff};
#[cfg(test)]
use lower::{lowering_error, runtime_pass_cache_keys, shader_binding_role, shader_sampling_edge};
#[expect(
    unused_imports,
    reason = "the staged pass front door preserves its crate-visible color-filter contract"
)]
...
#[cfg_attr(
    ...
    expect(
        unused_imports,
        reason = "the staged pass front door preserves its crate-visible runtime model contract"
    )
)]
pub(crate) use model::{
...
#[expect(
    unused_imports,
    reason = "the staged pass front door preserves its crate-visible texture-binding contract"
)]
...
#[cfg_attr(
    ...
    expect(
        unused_imports,
        reason = "the staged pass front door preserves its crate-visible prepared-binding contract"
    )
)]
...
#[cfg(test)]
use super::{Point, Rect, command::RenderClip, command::RenderCommands};
