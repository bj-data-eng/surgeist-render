...
pub(crate) use lower::{
    ...
    GraphLoweringResolvedAlphaMaskComposition,
};
use validate::{validate_graph_for_lowering, validate_semantic_frame_graph};

pub(crate) use bounds::LogicalBounds;
use bounds::{FrameSpatialPlan, SemanticContributionDomain, SemanticSourceContribution};
#[cfg(test)]
use bounds::{NonEmptyFrameSpatialPlan, PositiveDeviceExtent, SignedDeviceOrigin};
#[cfg(test)]
use filter::{
...
use graph::{
    ...
    SemanticCompositeKind, SemanticImportKind, SemanticPassId, SemanticPassIntent,
    SemanticPassResult, SemanticResourceDescriptor, SemanticResourceId, SemanticResourceProducer,
    SemanticResourceRole, SemanticVelloSpanScope, WorkingImageInitialization,
};
#[cfg(test)]
...
#[cfg(test)]
use super::{
    command::LayerPassPlan,
    filter::ColorClampBoundary,
    geometry::{PhysicalSize, Point},
