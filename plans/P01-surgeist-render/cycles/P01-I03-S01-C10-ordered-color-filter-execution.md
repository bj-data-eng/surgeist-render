...
- Final review reopened the cycle after the T6 correction because the mandatory repository-wide `clippy::too_many_lines` ceiling exposed pre-existing functions above 100 lines. T7 is one atomic behavior-preserving integration fix limited to decomposing that deterministic lint inventory; it changes no rendering, diagnostic, capability, or public contract.
...
| Modules | `filter.rs` owns finite scalar lowering; `frame.rs`/`pass.rs` own closed graph/runtime phases; `shader.rs` owns operation bytes and checked program objects; backend/transaction/renderer own atomic execution; `reference.rs` remains test-only. T7 splits over-limit functions into private cohesive helpers without moving ownership or semantics. |
...
**T7. Bound repository function sizes**
