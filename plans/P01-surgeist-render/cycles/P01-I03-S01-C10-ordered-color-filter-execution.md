...
## Header
- Cycle: `C10`; owner: `surgeist-render`; status: `complete`.
...
- Area/outcome: every tracked Rust file reported by the captured pre-change four-feature C10 Clippy inventory; split each unique function above the 100-line ceiling into private cohesive helpers while preserving module ownership, phases, control flow, diagnostics, resource lifetimes, test names and assertions, test-only gates, and public API. The captured diagnostic source locations, rather than an embedded count, are the authoritative finite T7 worklist.
