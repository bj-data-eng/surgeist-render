...
- Final review reopened the cycle after the T6 correction because the mandatory repository-wide `clippy::too_many_lines` ceiling exposed pre-existing functions above 100 lines. The post-task integration correction is one atomic behavior-preserving fix limited to decomposing that deterministic lint inventory; it changes no rendering, diagnostic, capability, or public contract.
...
| Modules | `filter.rs` owns finite scalar lowering; `frame.rs`/`pass.rs` own closed graph/runtime phases; `shader.rs` owns operation bytes and checked program objects; backend/transaction/renderer own atomic execution; `reference.rs` remains test-only. The integration correction splits over-limit functions into private cohesive helpers without moving ownership or semantics. |
...
cargo_c10 clippy -p surgeist-render --all-targets -- -F unsafe-code -D warnings -W clippy::too_many_lines
cargo_c10 test -p surgeist-render --features render-window
cargo_c10 clippy -p surgeist-render --all-targets --features render-window -- -F unsafe-code -D warnings -W clippy::too_many_lines
cargo_c10 test -p surgeist-render --features render-web
cargo_c10 clippy -p surgeist-render --all-targets --features render-web -- -F unsafe-code -D warnings -W clippy::too_many_lines
cargo_c10 test -p surgeist-render --features render-window,render-web
cargo_c10 clippy -p surgeist-render --all-targets --features render-window,render-web -- -F unsafe-code -D warnings -W clippy::too_many_lines
...
**Bound repository function sizes**
- Area/outcome: every tracked Rust file reported by the four-feature C10 Clippy matrix; split each function above the 100-line ceiling into private cohesive helpers while preserving module ownership, phases, control flow, diagnostics, resource lifetimes, test names and assertions, test-only gates, and public API. The measured pre-change inventory is 100 default/render-web functions and 107 when `render-window` is enabled, including 56 default-feature functions outside `src/tests.rs` and 44 inside it.
- Characterization/acceptance: the complete pre-change four-feature test matrix passes while the strict Clippy matrix fails only on the retained over-limit inventory outside the corrected T6 range. After implementation, all four strict Clippy feature states are warning-free, the exact test-name inventory and every existing assertion remain present and green, all T1-T6 focused evidence and completion guards remain green, and package behavior, GPU output, failures, capability values, public exports, dependencies, shaders, and unsafe absence remain unchanged. No lint allowance or expectation is added.
- Commands: capture the exact pre-change strict-lint and test-name inventories; run the complete package tests before and after the refactor; compare the exact test-name inventory; run the complete four-feature test and strict Clippy matrices, all T1-T6 named evidence and completion guards, then `C10-CHECK`.
- Coordination/review: this cycle-owned correction follows clean T6 and receives a fresh focused-fix worker, one exact attributed integration range, and a fresh task-style review before the cycle can return to complete status. Commit: `refactor(repo): bound repository function sizes`.

## Completion
- Require all six ordered task ranges and clean fresh task reviews plus the clean exact integration-correction review. The coordinator then changes only this plan status from `in_progress` to `complete` and commits that status separately. Run `C10-CHECK` and every T1-T6 named test with its stated feature set. Require exact extent/origin, S34 high/reduced evidence, one transaction submission, atomic headless/presented delivery, stable reuse, zero-budget cleanup, exact C11+ diagnostics, a warning-free 100-line Clippy ceiling across all four feature states, unchanged test-name inventory through the integration correction, and a clean worktree.
