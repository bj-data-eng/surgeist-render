...
CARGO_NET_OFFLINE=true cargo clippy -p surgeist-render --all-targets -- -F unsafe-code -D warnings -D clippy::too_many_lines
...
- The immutable reviewed head is published to authority `origin/main`, fresh
  ...
  evidence is retained, and `close_cycle` records C13 as closed.
...
- After C13 closure, the coordinator retains an explicit C14 planning packet
  ...
  platform/docs/example/root work. This handoff is coordination evidence, not a
  Rust test or build input.
