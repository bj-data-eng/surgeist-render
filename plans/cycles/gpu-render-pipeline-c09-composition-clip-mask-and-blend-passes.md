...
## Header
- Cycle: `C09`; owner: `surgeist-render`; status: `reviewed`.
...
```
No helper installs, updates, or acquires software. Every behavior task first adds its named test and the narrowest `#[cfg(test)]` observation needed, demonstrates the stated assertion RED, then implements it. Adapter absence, setup error, panic-before-assertion, or unrelated failure is not RED. Every worker runs its focused commands and `C09-CHECK`; the coordinator records its exact commit span and obtains a fresh task review before advancing.
...
- RED/acceptance: `c09_capabilities_name_only_gpu_semantics_and_keep_broad_masks_diagnostic` fails only at `C09 capability surface is stale or overclaims broad support`; `repeated_masked_and_blended_frames_reuse_resources_without_growth_or_readback` fails only at `C09 resources grow or enter readback`; `budget_zero_releases_composition_resources_without_changing_pixels` fails only at `zero retention changed composition pixels or kept idle resources`; `renderer_dispatch_routes_c08_and_c09_to_gpu_but_future_passes_to_typed_diagnostics` fails only at `dispatch has no exact C09 boundary`. Pass proves every old operation/accessor absent, resolved mask/image/composite/nested-opacity true, planning/resource facts true, future GPU execution and broad mask/filter/backdrop false with exact operations, stable counts after warm-up, exact pixels, one submission, retained upload reuse by key, deterministic coverage/effect cleanup, terminal-device release, and no CPU materialization/re-upload route.
